...
export const CreateMatch = ({ CloseTab }: { CloseTab: () => void }) => {
    ...
    const [players, setPlayers] = useState<any[]>([{ userUid: user?.uid, profileId: selectedProfile?.id, status: "owner", name: selectedProfile?.name, photoUrl: selectedProfile?.photoUrl || "" }]);
